...
function buildDateRange(): { iso: string; display: string; dayName: string }[] {
  ...
  return Array.from({ length: 14 }, (_, i) => {
    ...
    return { iso, display, dayName };
  });
}

const DATE_OPTIONS = buildDateRange();
...
export default function TripSearch({
  ...
  return (
    ...
    >
      {/* ── Top controls row: Charter type, Anglers, Duration ── */}
      <div
      ...
      <div className="px-5 py-4">
        ...
        >
          {/* Departure City */}
          <div
            className="flex-1 flex items-center gap-3 px-4 py-3 min-w-0 group relative"
            style={{ borderBottom: '1px solid #1e2a42' }}
          >
            {/* Dot icon */}
            ...
            />
            {/* Swap icon (decorative, positioned between city and species) */}
            <div
          ...
          <div className="flex-1 flex items-center gap-3 px-4 py-3 min-w-0">
            ...
            <Dropdown
              items={[
                ...
                ...DATE_OPTIONS.map(d => ({ label: `${d.dayName}, ${d.display}`, value: d.iso })),
              ]}
              value={selectedDate ?? ''}
              onSelect={val => onDateChange(val === '' ? null : val)}
